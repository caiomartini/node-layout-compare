...
export class ImageCompareService {
    ...
    public compareDirectories(): Promise<ImageCompareResult[]> {
        return new Promise<ImageCompareResult[]>((resolve, reject) => {
            ...
            try {
                const baseImages = DirectoryTree(this.baseImagesDirectory, { extensions: this.extension });                
                const imagesToCompare = DirectoryTree(this.imagesDirectory, { extensions: this.extension });
                if (baseImages.children) {
                    baseImages.children.forEach((baseImage: any) => {
    ...
    private compareFile(baseImage: any, imagesToCompare: any, threshold: number): Promise<ImageCompareResult> {
        return new Promise<ImageCompareResult>((resolve) => {
            ...
            const imageToCompare = imagesToCompare.children.filter((f: any) => f.name === baseImage.name);
            if (imageToCompare[0]) {
                try {
